...
require_once __DIR__ . '/class/pdf/pdf_demandepaiement_apc.modules.php';
...
$object = new ApcDemandePaiement($db);
$extrafields = new ExtraFields($db);
$extralabels = $extrafields->fetch_name_optionals_label('apclogistics_demandepaiement');
...
$permissiontoread     = !empty($user->rights->apclogistics->demandepaiement->read);
$permissiontocreate   = !empty($user->rights->apclogistics->demandepaiement->create);
$permissiontoedit     = !empty($user->rights->apclogistics->demandepaiement->edit);
$permissiontodelete   = !empty($user->rights->apclogistics->demandepaiement->delete);
$permissiontovalidate = !empty($user->rights->apclogistics->demandepaiement->validate);

if ($action === 'create' && !$permissiontocreate) accessforbidden();
if (empty($permissiontoread) && $action !== 'create') accessforbidden();

$error = 0;
$backtopage = DOL_URL_ROOT . '/custom/apclogistics/demandepaiement_list.php';
$form = new Form($db);
$formother = new FormOther($db);

if ($action === 'add' && $permissiontocreate && !$error && $user->valid && $token && $_SERVER['REQUEST_METHOD'] === 'POST') {
    $dd = dol_mktime(12,0,0, GETPOST('date_dpaimonth', 'int'), GETPOST('date_dpaiday', 'int'), GETPOST('date_dpaiyear', 'int'));
    $object->date_dpai        = $db->idate($dd);
    $object->objet            = GETPOST('objet', 'alphanohtml');
    $object->compte           = GETPOST('compte', 'alphanohtml');
    $object->cas_usage        = (int)GETPOST('cas_usage', 'int');
    ...
    $object->beneficiaire_nom    = GETPOST('beneficiaire_nom', 'alphanohtml');
    ...
    if (empty($object->date_dpai) || $object->date_dpai === '1970-01-01') { setEventMessages($langs->trans('ErrorFieldRequired', $langs->trans('DPAIDate')), null, 'errors'); $error++; }
    if ($object->cas_usage <= 0 || $object->cas_usage > 3) { setEventMessages($langs->trans('ErrorFieldRequired', $langs->trans('DPAICasUsage')), null, 'errors'); $error++; }
        ...
        if ($res > 0) {
            ...
            if (is_array($lignes)) {
                foreach ($lignes as $line) {
                    ...
                    $ln = new ApcDemandePaiementLine($db);
                    $ln->fk_demandepaiement = $object->id;
                    $ln->no_ligne           = $no++;
            ...
            $object->update($user);
            header('Location: ' . DOL_URL_ROOT . '/custom/apclogistics/demandepaiement_card.php?id=' . $object->id);
            exit;
...
if ($action === 'update' && $permissiontoedit && !$error && $user->valid && $token) {
    ...
    if ($res > 0) {
        ...
        if (is_array($lignes)) {
            foreach ($lignes as $line) {
                if (empty($line['depense_label']) && (float)$line['montant'] <= 0) continue;
                $nl = new ApcDemandePaiementLine($db);
                $nl->fk_demandepaiement = $object->id;
                $nl->no_ligne           = $no++;
...
if ($action === 'confirm_delete' && $confirm === 'yes' && $permissiontodelete && $token) {
    ...
    if ($res > 0) { header('Location: ' . DOL_URL_ROOT . '/custom/apclogistics/demandepaiement_list.php'); exit; }
    else setEventMessages($object->error, $object->errors, 'errors');
}

if ($action === 'generate_pdf' && $id > 0) {
    $g = new pdf_demandepaiement_apc($db);
...
$title = ($action === 'create' ? $langs->trans('NewDPAI') : ($object->ref ?: $langs->trans('DPAITitle')));
llxHeader('', $title, '', '', 0, 0, array('/custom/apclogistics/js/apclogistics.js'), array('/custom/apclogistics/css/apclogistics.css'));

$head = array();
$head[0][0] = DOL_URL_ROOT . '/custom/apclogistics/demandepaiement_card.php?id=' . ($object->id ?: 0) . '&action=' . ($action ?: 'view');
$head[0][1] = $langs->trans('TabCard');
$head[0][2] = 'tabcard';
$head[1][0] = DOL_URL_ROOT . '/custom/apclogistics/demandepaiement_card.php?id=' . ($object->id ?: 0) . '&action=links';
$head[1][1] = $langs->trans('TabLinks');
$head[1][2] = 'tablinks';
$head[2][0] = DOL_URL_ROOT . '/custom/apclogistics/demandepaiement_card.php?id=' . ($object->id ?: 0) . '&action=audit';
...
if ($action === 'create' || $action === 'edit') {
    ...
    print '<tr><td class="titlefieldcreate fieldrequired">' . $langs->trans('DPAIRef') . '</td><td>';
    ...
    print '<tr><td class="fieldrequired">' . $langs->trans('DPAIDate') . '</td><td>'
        . $form->select_date($obj->date_dpai ? $obj->date_dpai : -1, 'date_dpai', 0, 0, 1, '', 1, 0) . '</td>';
    ...
    print '<tr><td class="fieldrequired">' . $langs->trans('DPAICasUsage') . '</td><td colspan="3">';
    $cu = (int)($obj->cas_usage ? $obj->cas_usage : 1);
    for ($c = 1; $c <= 3; $c++) {
        print '<label style="margin-right:18px;"><input type="radio" name="cas_usage" value="'.$c.'"'.($cu===$c?' checked':'').'> <b>'.$langs->trans('DPAICas'.$c).'</b></label>';
    }
    print '<br><small class="opacitymedium">' . $langs->trans('DPAICasUsageDesc') . '</small>';
    print '</td></tr>';

    print '<tr><td>' . $langs->trans('DPAIBeneficiaire') . '</td><td>'
        . '<input type="text" size="50" name="beneficiaire_nom" value="' . dol_escape_htmltag($obj->beneficiaire_nom) . '"></td>';
    print '<td>' . $langs->trans('DAVCompte') . '</td><td>'
        . '<input type="text" size="40" name="compte" value="' . dol_escape_htmltag($obj->compte) . '"></td></tr>';

    print '<tr><td class="fieldrequired">' . $langs->trans('DAVModePaiement') . '</td><td>';
    ...
    print '<h3 style="margin-top:18px;">' . $langs->trans('DPAIColDepense') . 's</h3>';
    print '<table class="noborder centpercent" id="apc-dpai-lines-table">';
    print '<thead><tr class="liste_titre">';
    print '<th class="width40" style="width:40px;">N°</th>';
    print '<th style="min-width:240px;">' . $langs->trans('DPAIColDepense') . ' *</th>';
    print '<th>' . $langs->trans('DPAIColProjet') . '</th>';
    print '<th>' . $langs->trans('DPAIColBudget') . '</th>';
    print '<th style="width:160px;" class="right">' . $langs->trans('DPAIColMontant') . '</th>';
    print '<th style="width:40px;">&nbsp;</th>';
    print '</tr></thead>';
    print '<tbody id="apc-dpai-lines-body">';
    ...
    foreach ($lines as $ln) {
        ...
        print '<td class="apc-dpai-no center">' . ($idx+1) . '</td>';
    ...
    print '</tbody>';
    print '<tfoot><tr class="liste_total"><td colspan="4" class="right"><b>' . $langs->trans('DPAIGeneralTotal') . '</b></td>'
        . '<td class="right apc-money" id="apc-dpai-total">0,00</td><td>&nbsp;</td></tr></tfoot>';
    print '</table>';
    ...
    print ' <a class="button button-cancel" href="' . DOL_URL_ROOT . '/custom/apclogistics/demandepaiement_list.php">' . $langs->trans('Cancel') . '</a>';
    print '</div>';
    print '</form>';
    ...
    $(document).ready(function() {
        ...
        $("#apc-add-line").on("click", function() {
            ...
            var html = "<tr class=\"apc-line-row\">"
                ...
                + "</tr>";
            $("#apc-dpai-lines-body").append(html);
            renumber(); calcTotal();
        });
        $(document).on("click", ".apc-rm-line", function() {
            if ($("#apc-dpai-lines-body tr").length > 1) { $(this).closest("tr").remove(); renumber(); calcTotal(); }
        });
        calcTotal();
    });
    </script>';
    ...
    print '<table class="border centpercent">';
    print '<tr><td class="titlefield">' . $langs->trans('DPAIRef') . '</td><td class="valeur"><b>' . $object->ref . '</b></td>'
        . '<td class="titlefield">' . $langs->trans('FieldStatus') . '</td><td class="valeur">' . $object->getStatusBadge() . '</td></tr>';
    print '<tr><td>' . $langs->trans('DPAIDate') . '</td><td>' . dol_print_date($db->jdate($object->date_dpai), 'day') . '</td>'
    ...
    print '<tr><td>' . $langs->trans('DPAICasUsage') . '</td><td><b>' . $object->getCasUsageLabel() . '</b> (Cas ' . (int)$object->cas_usage . ')</td>';
    print '<td>' . $langs->trans('DPAIBeneficiaire') . '</td><td>' . dol_escape_htmltag($object->beneficiaire_nom) . '</td></tr>';
    ...
    if (!empty($object->date_paiement_effectif)) {
        print '<tr><td>' . $langs->trans('DPAIDatePaiementEffectif') . '</td><td colspan="3">' . dol_print_date($db->jdate($object->date_paiement_effectif), 'day') . '</td></tr>';
    }
    print '<tr><td>' . $langs->trans('DPAIDemandeur') . '</td><td>' . dol_escape_htmltag($object->demandeur_nom)
        ...
        . '</td>';
    print '<td>' . $langs->trans('DPAIVerificateur') . '</td><td>' . dol_escape_htmltag($object->verif_nom)
        ...
        . '</td></tr>';
    print '<tr><td>' . $langs->trans('DPAIApprobateur') . '</td><td colspan="3">' . dol_escape_htmltag($object->approb_nom)
    ...
    print '<th>' . $langs->trans('DPAIColDepense') . '</th>';
    print '<th>' . $langs->trans('DPAIColProjet') . '</th>';
    print '<th>' . $langs->trans('DPAIColBudget') . '</th>';
    print '<th class="right" style="width:160px;">' . $langs->trans('DPAIColMontant') . '</th>';
    ...
    print '</tbody><tfoot><tr class="liste_total"><td colspan="4" class="right"><b>' . $langs->trans('DPAIGeneralTotal') . '</b></td>'
        . '<td class="right apc-money">' . price($total, 0, $langs, 0, 0, -1, $object->devise ?: $conf->currency) . '</td></tr></tfoot>';
    print '</table>';

    if ($permissiontovalidate && (int)$object->status < ApcDemandePaiement::STATUS_VALIDATED) {
        print '<h3 style="margin-top:18px;">' . $langs->trans('AUDSignature') . '</h3>';
        foreach (array(0=>'DPAIDemandeur', 1=>'DPAIVerificateur', 2=>'DPAIApprobateur') as $lvl => $key) {
            $signed = false;
    ...
    print '<div class="tabsAction" style="margin-top:24px;">';
    if ($id > 0) print '<a class="butAction" href="' . DOL_URL_ROOT . '/custom/apclogistics/demandepaiement_card.php?id=' . $object->id . '&action=generate_pdf&token=' . newToken() . '">' . $langs->trans('PDFGenerateBtn') . '</a>';
    if ($permissiontoedit && (int)$object->status < ApcDemandePaiement::STATUS_VALIDATED) print '<a class="butAction" href="' . DOL_URL_ROOT . '/custom/apclogistics/demandepaiement_card.php?id=' . $object->id . '&action=edit">' . $langs->trans('Modify') . '</a>';
    if ($permissiontodelete) print '<a class="butActionDelete" href="' . DOL_URL_ROOT . '/custom/apclogistics/demandepaiement_card.php?id=' . $object->id . '&action=delete">' . $langs->trans('Delete') . '</a>';
    print '</div>';
}
...
if ($action === 'audit' && $object->id > 0) {
    $audits = ApcAuditLog::fetchForEntity($db, 'dpai', $object->id, 200);
