 ...
 *
 * etatbesoin_list.php — Listing paginé des États de Besoin APC
 * Filtres : statut, date début/fin, mot-clé dans objet/demandeur
 */
...
$contextpage = GETPOST('contextpage', 'aZ') ? GETPOST('contextpage', 'aZ') : 'apclogisticsetatbesoinlist';
...
if (empty($sortfield)) $sortfield = 'eb.date_creation';
if (empty($sortorder)) $sortorder = 'DESC';
if ($page < 0) $page = 0;
$offset = $page * $limit;

$title = $langs->trans('APCLogisticsMenu_etatbesoin');
...
$sql = "SELECT eb.rowid, eb.ref, eb.date_eb, eb.objet, eb.status, eb.total_ht,"
     . " eb.fk_user_demandeur, eb.signataire_nom_d,"
     . " eb.date_signature_approb";
$sql_count = "SELECT COUNT(eb.rowid)";
...
print '<td><input type="text" class="flat width100" name="search_ref" value="' . dol_escape_htmltag($search_ref) . '" placeholder="' . dol_escape_htmltag($langs->trans('FieldRef') . ' / Objet') . '"></td>';
$statusList = (new ApcEtatBesoin($db))->getStatusList();
...
print getTitleFieldOfList($langs->trans('FieldRef'), 0, $_SERVER["PHP_SELF"], 'eb.ref', '', '', $sortfield, $sortorder, '', 1) . "\n";
print getTitleFieldOfList($langs->trans('EBDate'), 0, $_SERVER["PHP_SELF"], 'eb.date_eb', '', '', $sortfield, $sortorder, '', 1) . "\n";
print getTitleFieldOfList($langs->trans('FieldObjet'), 0, $_SERVER["PHP_SELF"], 'eb.objet', '', '', $sortfield, $sortorder, '', 1) . "\n";
print getTitleFieldOfList($langs->trans('EBDemandeur'), 0, $_SERVER["PHP_SELF"], 'eb.signataire_nom_d', '', '', $sortfield, $sortorder, '', 1) . "\n";
print getTitleFieldOfList($langs->trans('FieldTotalHT'), 0, $_SERVER["PHP_SELF"], 'eb.total_ht', '', '', $sortfield, $sortorder, 'right ', 1) . "\n";
print getTitleFieldOfList($langs->trans('FieldStatus'), 0, $_SERVER["PHP_SELF"], 'eb.status', '', '', $sortfield, $sortorder, 'center ', 1) . "\n";
print '</tr>';

$ebtmp = new ApcEtatBesoin($db);
if ($num) {
    $i = 0;
    while ($i < min($num, $limit)) {
        $obj = $db->fetch_object($resql);
        if (!$obj) break;
        $ebtmp->id = $obj->rowid; $ebtmp->ref = $obj->ref; $ebtmp->status = $obj->status;
        print '<tr class="oddeven">';
        print '<td class="center"><input type="checkbox" class="flat checkforselect" name="toselect[]" value="' . $obj->rowid . '"></td>';
        print '<td>' . $ebtmp->getNomUrl(1) . '</td>';
        print '<td>' . dol_print_date($db->jdate($obj->date_eb), 'day') . '</td>';
        print '<td>' . dol_escape_htmltag($obj->objet) . '</td>';
        print '<td>' . dol_escape_htmltag($obj->signataire_nom_d) . '</td>';
        print '<td class="apc-money">' . price((float)$obj->total_ht, 0, $langs, 0, 0, -1, $conf->currency) . '</td>';
        print '<td class="center">' . $ebtmp->getStatusBadge() . '</td>';
        print '</tr>';
        $i++;
    }
} else {
...
// ------- Bouton création -------
if ($user->rights->apclogistics->etatbesoin->create) {
    print '<div class="tabsAction" style="margin-top:16px;">';
    print '<a class="butAction" href="' . DOL_URL_ROOT . '/custom/apclogistics/etatbesoin_card.php?action=create">' . $langs->trans('NewEtatBesoin') . '</a>';
    print '</div>';
}
